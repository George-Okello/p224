...
export default function ContentList({items, contentType,fallbackItemImage, viewMoreText="Learn More"}:ContentListProps) {
                ...
                >
                    ...
                    <Link href={urlPrefix +"/"+ item.uid} className='flex flex-col justify-between border-t border-t-slate-100 py-10  text-slate-200 md:flex-row'
                        ...
                        <div className='flex flex-col'>
                            ...
                            <div className='flex gap-3 text-orange-500 text-lg font-semibold'>
                                {item.tags.map((tag, index)=>(
                                    <span key={index}>{tag}</span>
                                ))}
                            </div>
                        </div>
                        <span className='ml-auto flex items-center gap-2 text-xl font-medium md:ml-0'>{viewMoreText}<MdArrowOutward className="inline-block" /></span>
                    </Link>
                </li>
            )}
            </>
